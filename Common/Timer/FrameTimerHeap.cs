...
//void Awake()
//{
//  m_ChangeTimeId = TimerHeap.AddTimer(500, 0, () => { Debug.Log("11111");});
//}
//
//void OnDestroy()
//{
//  TimerHeap.DelTimer(m_ChangeTimeId);
//}
...
#pragma warning disable 0414
        //Ensures that the TimerHelper will be created automatically upon start of the game.
        static private TimerManager timerHelper = TimerManager.Instance;
#pragma warning restore 0414
        ...
        public static void Tick()
        {
            ...
            while (m_queue.Count != 0)
            {
                ...
                lock (m_queueLock)
                    p = m_queue.Peek();
                if (m_unTick < p.UnNextTick)//没到timer间隔时间
                {
                    break;
                }
                lock (m_queueLock)
                    m_queue.Dequeue();//通过出队保证队列中元素都能被执行
                if (p.NInterval > 0)//轮询心跳测试
                {
                    p.UnNextTick += (ulong)p.NInterval;
                    lock (m_queueLock)
                        m_queue.Enqueue(p.NTimerId, p, p.UnNextTick);//下一次轮询间隔
                    p.DoAction();
                }
                else
                {
        ...
        private static T GetTimerData<T>(T p, uint start, int interval) where T : AbsTimerData
        {
            ...
            p.UnNextTick = m_unTick + 1 + start;//延时1s后调用
            return p;
        }

        public static void ReleaseVal()
        {
            Reset();
        }
    }
